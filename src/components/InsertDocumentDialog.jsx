...
import { useDocumentsInsertAPIMutation } from '../reducers/documents/api'
import LoadingSpinner from './LoadingSpinner'

function InsertDocumentDialog({ isOpenInsertDocument, handleInsertDialog, docName, docId, setRefresh }) {
    const [documentName, setDocumentName] = useState('')
    const [document, setDocument] = useState(null)
    const { userInfo } = useSelector(state => state.user)
    const [errorMessage, setErrorMessage] = useState('')

    const [documentsInsertAPI, { isLoading: isLoadingUpdate, error: errorUpdate, isError: isErrorUpdate }] = useDocumentsInsertAPIMutation()
    ...
    const handleUpdate = async () => {
        ...
        const result = await documentsInsertAPI({ token: userInfo.token, documentId: docId, name: documentName, document: document }).unwrap()
        if (result.success) {
    ...
    return (
        <Transition show={isOpenInsertDocument} as={Fragment}>
            <HeadlessDialog onClose={handleInsertDialog} className="relative z-50">
                <Transition.Child
                    as={Fragment}
